...
we will store it together with a hash in each node (as shown in Figure~\ref{fig:merkle-tree}). The proof will only be valid if the Educator fills in the sizes correcctly, so there is no incentive for Educators to lie about the size of the tree.
