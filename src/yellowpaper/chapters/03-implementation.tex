...
These validation steps can prove that an Educator with a public key $PK_E$ issued a transaction $T_{priv}$ in one of its private blocks. One can attribute the $PK_E$ to some real-world educational institute by checking the Educator's certificate as described in section \ref{sec:cert}.
...
\item The Educators determines the sizes of the data they are going to disclose.
\item Based on the size of the data, the Archivist informs the Recruiter about the cost of the request.
\item The Recruiter pays the fees.
\item The Educators send the data to the Archivist.
\item The Archivist checks the blocks received from the Educator for the validity using the data from the Witnesses' chain, and forward this data to the Recruiter.
\item The fee is distributed between the affected parties.
\end{enumerate}
