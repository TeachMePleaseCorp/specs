...
we will store the size of the subtree with the hash in each node (as shown in \ref{fig:merkle-tree}). So every transaction disclosure will also verify the size. If an educator publishes digest of the tree with the wrong size, then any transaction won't pass validation.
